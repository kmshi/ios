...
#import "Sound.h"
#import "AppMobiWebView.h" // [appMobi added]
...
- (NSURL*) urlForResource:(NSString*)resourcePath
{
	...
	// [appMobi removed] NSString* filePath = [PhoneGapDelegate pathForResource:resourcePath];
	NSString* filePath = nil; // [appMobi added]
	...
	if (filePath == nil) {
		...
		} else if (NO && [resourcePath hasPrefix:DOCUMENTS_SCHEME_PREFIX]) { // [appMobi added]
		// [appMobi removed] } else if ([resourcePath hasPrefix:DOCUMENTS_SCHEME_PREFIX]) {			// [appMobi comment] this case won't be reached. there will be no
			// bundled resources in the binary. everything is in the app bundle.
			NSLog(@"Will use resource '%@' from the documents folder.", resourcePath);
			resourceURL = [NSURL URLWithString:resourcePath];
			
			// [appMobi removed] NSString* recordingPath = [NSString stringWithFormat:@"%@/%@", [PhoneGapDelegate applicationDocumentsDirectory], [resourceURL host]];
			NSString* recordingPath = [NSString stringWithFormat:@"%@/%@", webView.appDirectory, [resourceURL host]]; // [appMobi added]
		...
		// [appMobi removed] this case won't be reached. there will be no
		// bundled resources in the binary. everything is in the app bundle.
