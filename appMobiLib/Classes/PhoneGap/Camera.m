...
#import "AppConfig.h" // [appMobi added]
...
- (void)imagePickerController:(UIImagePickerController*)picker didFinishPickingMediaWithInfo:(NSDictionary*)info
{
	...
	if ([mediaType isEqualToString:(NSString*)kUTTypeImage])
	{
        ...
        if (self.pickerController.returnType == DestinationTypeFileUri){
			...
			// [appMobi removed] NSString* docsPath = [NSTemporaryDirectory() stringByStandardizingPath];
            NSString* docsPath = [webView.config.appDirectory copy]; // [appMobi added]
			NSError* err = nil;
			...
			// save file
            NSString *fileURL = [NSString stringWithFormat:@"%@/photo_%03d.%@", [webView webRoot], --i, self.pickerController.encodingType == EncodingTypePNG ? @"png" : @"jpg"]; // [appMobi added]
			if (![data writeToFile: filePath options: NSAtomicWrite error: &err]){
				result = [PluginResult resultWithStatus: PGCommandStatus_OK messageAsString: [err localizedDescription]];
				jsString = [result toErrorCallbackString:callbackId];
			}else{
				// [appMobi removed] result = [PluginResult resultWithStatus:PGCommandStatus_OK messageAsString:[NSURL fileURLWithPath:filePath]];
                result = [PluginResult resultWithStatus:PGCommandStatus_OK messageAsString:[NSURL URLWithString:fileURL]]; // [appMobi added]
