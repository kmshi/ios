...
- (BOOL)startSigningIn {
  ...
  BOOL didStart = [fetcher beginFetchWithDelegate:self didFinishSelector:@selector(requestFetcher:finishedWithData:error:)];
  if (didStart) {
...
- (void)startWebRequest {
  ...
  if ([token length] > 0) {
	...
	// Dirty hack to make sure they gave us the right URL, otherwise it hangs
	NSData *data = [NSData dataWithContentsOfURL:[request URL]];
	if( data == nil || [data length] <= 1 )
	{
		[self invokeFinalCallbackWithError:[NSError errorWithDomain:@"Invalid authorize endpoint." code:401 userInfo:nil]];
		return;
	}

    [delegate_ performSelector:webRequestSelector_ withObject:self withObject:request];
