...
- (void)replyToHTTPRequest
{
	...
	if(contentLength == 0)
	{
		NSLog(@"HTTP Server: Error 404 - %@", [uri description]);
...
- (NSString *)filePathForURI:(NSString *)path
{
	...
	NSURL *url;
	BOOL isamJS = [relativePath hasPrefix:@"_appMobi/"] && [relativePath hasSuffix:@".js"];
	...
	{
		BOOL isAppDir = NO;
		NSString *referer = [NSMakeCollectable(CFHTTPMessageCopyHeaderFieldValue(request, CFSTR("Referer"))) autorelease];
		if( referer != nil )
		{
			NSString *appbase = @"";
			NSRange range = [referer rangeOfString:@"localhost:58888"];
			if( range.length > 0 )
			{
				isAppDir = YES;
				appbase = [appbase stringByAppendingString:[referer substringFromIndex:range.location + range.length + 1]];
				range = [appbase rangeOfString:@"/"];
				NSString *app = [appbase substringToIndex:range.location];
				appbase = [appbase substringFromIndex:range.location + 1];
				range = [appbase rangeOfString:@"/"];
				NSString *rel = [appbase substringToIndex:range.location];
				appbase = [NSString stringWithFormat:@"%@/%@/", app, rel];
				relativePath = [NSString stringWithFormat:@"%@%@", appbase, relativePath];
		...
		if( isAppDir == NO )
		{
            NSRange range = [relativePath rangeOfString:@"_appMobi/"];
            NSString *jsfile = [relativePath substringFromIndex:range.location + 9];
            NSString *jsname = [jsfile substringToIndex:[jsfile length] - 3];
            
            if( [jsname compare:@"appmobi"] == NSOrderedSame )
            {
                relativePath = [[NSBundle mainBundle] pathForResource:@"appmobi_iphone" ofType:@"js"];
                relativePath = [relativePath substringToIndex:[relativePath length] - 17];
                url = [NSURL URLWithString:@"appmobi_iphone.js" relativeToURL:[NSURL fileURLWithPath:relativePath]];
                return [[url path] stringByStandardizingPath];
            }
            else
            {
                relativePath = [[NSBundle mainBundle] pathForResource:jsname ofType:@"js"];
                if( relativePath != nil )
                {
                    relativePath = [relativePath substringToIndex:[relativePath length] - [jsname length] - 3];
                    url = [NSURL URLWithString:jsfile relativeToURL:[NSURL fileURLWithPath:relativePath]];
                    return [[url path] stringByStandardizingPath];
                }
            }
		}
